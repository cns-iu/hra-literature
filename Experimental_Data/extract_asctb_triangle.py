...
def extract_entity_data(entity_data, entity_type, organ_key, row_id):
    table_data = []
    ...
    for entry in entity_data:
        ...
        # Append to table with row_id and entity_type
        table_data.append([row_id, id_value, rdfs_label, name, b_type, organ_key, entity_type])
...
# Initialize empty list to hold data for the new table with row_id and entity_type
table_with_row_id_data = []

# Define a mapping for entity_type abbreviations
entity_type_mapping = {'anatomical_structures': 'AS', 'cell_types': 'CT', 'biomarkers': 'B'}
...
for organ_key in data.keys():
    ...
    for entry in organ_data:
        ...
        for entity_type in ['anatomical_structures', 'cell_types', 'biomarkers']:
            ...
            entity_type_abbr = entity_type_mapping.get(entity_type, '')

            table_data = extract_entity_data(entity_data, entity_type_abbr, organ_key, row_id)
            table_with_row_id_data.extend(table_data)

# Create DataFrame and save it to CSV
table_with_row_id_columns = ['row_id', 'id', 'rdfs_label', 'name', 'b_type', 'organ', 'entity_type']
table_with_row_id_df = pd.DataFrame(table_with_row_id_data, columns=table_with_row_id_columns)

# Save to CSV
output_path = 'asctb_triangle.csv'
